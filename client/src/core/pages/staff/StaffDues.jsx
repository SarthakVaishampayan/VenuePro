...
export default function StaffDues() {
  ...
  const fetchDues = async () => {
    try {
      const { data } = await staffApi.get(`/dues?status=${statusFilter}`);
      setDues(data.data || []);
  ...
  const handlePay = async () => {
    ...
    try {
      ...
      await staffApi.post(`/dues/${payDue._id}/pay`, { amount, mode: payMode });
      setShowPay(false);
      fetchDues();
    } catch (err) {
  ...
  const handleWaive = async () => {
    ...
    setWaiving(true);
    try {
      await staffApi.post(`/dues/${waiveDue._id}/waive`);
      setShowWaive(false);
      fetchDues();
    } catch (err) {
