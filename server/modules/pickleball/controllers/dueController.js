...
import PickleballDue from '../models/Due.js';
import PickleballPayment from '../models/Payment.js';
...
async function updateSessionPaymentStatusFromDues(sessionId) {
  try {
    const allDuesForSession = await PickleballDue.find({ bookingSessionId: sessionId });
...
export const getAllDues = async (req, res, next) => {
  try {
    ...
    const dues = await PickleballDue.find(filter)
      .sort({ createdAt: -1 })
...
export const getDueById = async (req, res, next) => {
  try {
    const due = await PickleballDue.findOne({ _id: req.params.id, tenantId: req.tenantId })
      .populate('customerId', 'fullName phone')
...
export const createDue = async (req, res, next) => {
  try {
    ...
    const due = await PickleballDue.create({
...
export const payDue = async (req, res, next) => {
  try {
    ...
    const payment = await PickleballPayment.create({
      tenantId: req.tenantId,
...
export const getCustomerDues = async (req, res, next) => {
  try {
    const dues = await PickleballDue.find({
      tenantId: req.tenantId,
...
export const payCustomerDues = async (req, res, next) => {
  try {
    ...
    const pendingDues = await PickleballDue.find({
      tenantId: req.tenantId,
