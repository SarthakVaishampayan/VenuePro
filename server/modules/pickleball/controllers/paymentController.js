  ...
  try {
    ...
    const [payments, total] = await Promise.all([
      PickleballPayment.find(filter)
        ...
        .lean(),
      PickleballPayment.countDocuments(filter)
    ]);
...
export const recordPayment = async (req, res, next) => {
  try {
    ...
    if (bookingSessionId) {
      const modelNames = ['PickleballBookingSession', 'BookingSession', 'CricketFootballBookingSession', 'GamingZoneBookingSession'];
...
export const getDailySummary = async (req, res, next) => {
  try {
    ...
    const payments = await PickleballPayment.find({
      tenantId: req.tenantId,
