...
import GamingZoneDue from '../models/Due.js';
...
async function updateSessionPaymentStatusFromDues(sessionId) {
  try {
    const allDuesForSession = await GamingZoneDue.find({ bookingSessionId: sessionId });
...
export const getAllDues = async (req, res, next) => {
  try {
    ...
    const dues = await GamingZoneDue.find(filter)
      .sort({ createdAt: -1 })
...
export const getDueById = async (req, res, next) => {
  try {
    const due = await GamingZoneDue.findOne({ _id: req.params.id, tenantId: req.tenantId })
      .populate('customerId', 'fullName phone')
...
export const createDue = async (req, res, next) => {
  try {
    ...
    const existingDue = await GamingZoneDue.findOne({
      bookingSessionId,
      customerId,
    ...
    const session = await mongoose.model('GamingZoneBookingSession').findById(bookingSessionId).lean();
    if (!session) {
    ...
    const due = await GamingZoneDue.create({
...
export const getCustomerDues = async (req, res, next) => {
  try {
    const dues = await GamingZoneDue.find({
      tenantId: req.tenantId,
...
export const payCustomerDues = async (req, res, next) => {
  try {
    ...
    const pendingDues = await GamingZoneDue.find({
      tenantId: req.tenantId,
    ...
    for (const due of pendingDues) {
      ...
      if (payForThisDue > 0) {
        await GamingZonePayment.create({
          tenantId: req.tenantId,
          bookingSessionId: due.bookingSessionId,
          customerId: due.customerId,
          dueId: due._id,
      ...
      const session = await mongoose.model('GamingZoneBookingSession').findById(due.bookingSessionId);
      if (session && ['pending', 'due', 'partial'].includes(session.paymentStatus)) {
        const allDuesForSession = await GamingZoneDue.find({ bookingSessionId: due.bookingSessionId });
