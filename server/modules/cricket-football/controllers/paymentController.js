...
export const getAllPayments = async (req, res, next) => {
  try {
    ...
    const [payments, total] = await Promise.all([
      CricketFootballPayment.find(filter)
        .sort({ createdAt: -1 })
        .skip(skip)
        .limit(limitNum)
        ...
        .lean(),
      CricketFootballPayment.countDocuments(filter)
    ]);
...
export const recordPayment = async (req, res, next) => {
  try {
    ...
    if (bookingSessionId) {
      const modelNames = ['CricketFootballBookingSession', 'BookingSession', 'PickleballBookingSession', 'GamingZoneBookingSession'];
...
export const getDailySummary = async (req, res, next) => {
  try {
    ...
    const payments = await CricketFootballPayment.find({
      tenantId: req.tenantId,
