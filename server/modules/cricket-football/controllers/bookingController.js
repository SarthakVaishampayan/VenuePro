...
export const startSession = async (req, res, next) => {
  try {
    ...
    const turf = await Turf.findOne({ _id: resourceId, tenantId: req.tenantId });
    if (!turf) {
    ...
    if (activeSession) {
      return error(res, { statusCode: 400, message: 'Turf already has an active session', code: 'ALREADY_ACTIVE' });
    }
    ...
    const session = await BookingSession.create({
      tenantId: req.tenantId,
      resourceId,
      resourceNameSnapshot: turf.name,
      ...
      rateSnapshot: { dayPrice: turf.dayPrice, nightPrice: turf.nightPrice },
      dayPrice: turf.dayPrice,
      nightPrice: turf.nightPrice,
      pricingMode
    });

    turf.status = 'occupied';
    await turf.save();
